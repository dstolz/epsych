...
if append % use old threshold
    ...
    reject = spikes.info.detect.reject;
...
for j = 1:num_trials
    ...
    for k = 1:length(crossings)
        ...
        %kp {
        % skip crossings that represent events above/below reject threshold
        this_waveform =  data{j}(indices, :) ;                      
        
        if any(this_waveform < reject | this_waveform > -reject)
            reject_k = [reject_k k];
            continue
        end;
        ik=ik+1;    %kp }
    ...
    % update spiketimes, trials
    channel(reject_k)   = [];           %kp
    crossings(reject_k) = [];           %kp
