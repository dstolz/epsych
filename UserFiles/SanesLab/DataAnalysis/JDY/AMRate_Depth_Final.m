function AMRate_Depth_Final
%---Script that analyzes behavior data from AM Rate Detection Task.
%Created 06/14/2017 JDY. Gathers all behavior session matlab files and
%saves accordingly.
clear all
close all
clc
...
Pname		=	getPath(Ephys,CHL,CHLEphys,Gerbil,NoFringe);
...
% 	id			=	[Gerbil '-' Date{a}];
	...
	%---Save Data---%
	if( Save )
		save([ Pname '/Saved/' num2str(RATE) 'Hz/' id],'Data')
		disp('Saved Data!')
	end
% 	pause
% 	close all
...
%---Locals---%
function Pname = getPath(Ephys,CHL,CHLEphys,Gerbil,NoFringe)
% if( Ephys )
...
% end
if( Ephys )
	Pname		=	'/Volumes/JUSTIN EXHD/PROJECTS/AM RATE/Ephys Behavior/';
elseif( CHL )
	Pname		=	'/Volumes/JUSTIN EXHD/PROJECTS/AM RATE/BehaviorCHL/';
elseif( CHLEphys )
	Pname		=	'/Volumes/JUSTIN EXHD/PROJECTS/AM RATE/Ephys Behavior/CHL/';
else
	Pname		=	'/Volumes/JUSTIN EXHD/PROJECTS/AM RATE/Behavior/';
end
if( NoFringe )
	Pname			=	[Pname Gerbil '/Depth2/'];
else
	Pname			=	[Pname Gerbil '/Depth/'];
end
...
X		=	[0.13;0.25;0.35;0.50;0.71];
pFA		=	0.21;
pHits	=	[ 0.21;0.38;0.75;0.95;0.95];
D		=	[X (Ngo.*pHits) N];
...
if ~isfield(options,'confP'),                options.confP          = 0.95; end%[0.95,0.9,.68]; end
