function DATA = pp_plot_rasters_AMBehavior_OnTask(path, subject, session, block, channel, clu)
%
...
box on
if( clu(1,2) == 2 )
	title(['On-Task; Ch: ' num2str(channel) '; dBSPL: ' num2str(unique_stim(ks,2)) '; SU'])
else
	title(['On-Task; Ch: ' num2str(channel) '; dBSPL: ' num2str(unique_stim(ks,2)) '; MU'])
end
% axis square
...
ylim([0 cntt])
hold off
title(['On-Task PSTH; bin width: ' num2str(bin) ' ms'])
% axis square
...
h(1)=plot(nan,nan,'ko','MarkerFaceColor','k');
h(2)=plot(nan,nan,'ko','MarkerFaceColor','w');
legend(h,'On-Task','Off-Task','Location','SouthWest')
legend('boxoff')
