...
%     data.ylims = [min(spikes.waveforms(:)) max(spikes.waveforms(:))];
	data.ylims	=	[-3.5e-04 3.5e-04];
%     up	=	mean(spikes.waveforms(:)) + std(spikes.waveforms(:));
% 	dn	=	mean(spikes.waveforms(:)) - std(spikes.waveforms(:));
% 	data.ylims = [-0.01 0.01];
% 	data.ylims	=	[spikes.info.detect.thresh*-10 spikes.info.detect.thresh*10];
