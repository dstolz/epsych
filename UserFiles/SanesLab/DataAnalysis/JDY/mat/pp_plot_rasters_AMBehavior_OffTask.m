function DATA = pp_plot_rasters_AMBehavior_OffTask(path, subject, session, block, channel, clu)
%
...
% 	figure(1)
	...
	if( ~isempty(cnt) )
		...
		hold on
		if( ks == 1 )
			plot(  raster_x  ,  raster_Y  , 'r.','MarkerSize',8)
		else
...
box on
if( clu(1,2) == 2 )
	title(['Off-Task; Ch: ' num2str(channel) '; dBSPL: ' num2str(unique_stim(ks,2)) '; SU'])
else
	title(['Off-Task; Ch: ' num2str(channel) '; dBSPL: ' num2str(unique_stim(ks,2)) '; MU'])
end
% axis square
...
ylim([0 cntt])
hold off
title(['Off-Task PSTH; bin width: ' num2str(bin) ' ms'])
% axis square
...
for i=1:N
	...
	if( i == 1 )
		...
		hold on
		plot(AM(i),sr,'ro','MarkerFaceColor','w','MarkerSize',10)
	else
		plot([AM(i) AM(i)],[sr + se sr - se],'k-','Color',[0.50 0.50 0.50])
		hold on
		plot(AM(i),sr,'ko','MarkerFaceColor','w','MarkerSize',10,'Color',[0.50 0.50 0.50])
	end
	hold on
	spkrate(1,i)	=	sr;
end
...
for i=1:N
	...
	if( i == 1 )
		...
		hold on
		plot(AM(i),pr,'ro','MarkerFaceColor','w','MarkerSize',10)
	else
		plot([AM(i) AM(i)],[pr + se pr - se],'k-')
		hold on
		plot(AM(i),pr,'ko','MarkerFaceColor','w','MarkerSize',10)
	end
	hold on
	power(1,i)	=	pr;
end
...
DP          =   DP(:,end);
plot(AM,DP,'k-','Color',[0.50 0.50 0.50])
hold on
plot(AM,DP,'ko','MarkerFaceColor','w','MarkerSize',10,'Color',[0.50 0.50 0.50])
xlim([2 14])
maxy		=	3;
ylim([0 maxy])
set(gca,'FontSize',16)
...
plot(AM(2:end),VS(2:end),'k-','Color',[0.50 0.50 0.50])
hold on
plot(AM(2:end),VS(2:end),'ko','MarkerFaceColor','w','MarkerSize',10,'Color',[0.50 0.50 0.50])
plot(AM(1),VS(1),'ro','MarkerFaceColor','w','MarkerSize',10,'Color',[1 0 0.50])
xlim([2 14])
maxy		=	nanmax(VS);
if( maxy < 0.50 )
    ylim([0 0.50])
