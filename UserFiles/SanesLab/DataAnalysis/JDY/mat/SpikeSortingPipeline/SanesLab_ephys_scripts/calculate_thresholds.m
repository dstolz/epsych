function [thresh, reject] = calculate_thresholds( Phys, segment_length_s, Threshold, win, fs )
...
%             plot( Phys(it,:,channel-1+isp) + (isp*8)*10^-4 ,'Color',[0.4 0.4 0.4])
            plot( Phys(it,Win(1):Win(2),channel-1+isp) + (isp*8)*10^-4 ,'Color',[0.4 0.4 0.4])
			hold on
        end
        set(gca, 'XTick',[], 'YTick',[])
...
for ich = 1:size(Phys,3)
	try
%     std_cln(1,ich) = nanmean(nanstd(Phys( all_clean(ich,:) ,:,ich),0,2));	%%%can try to switch to median%%%
       std_cln(1,ich) = nanmean(nanstd(Phys( all_clean(ich,:) ,Win(1):Win(2),ich),0,2));	%%%can try to switch to median%%%
	catch
		keyboard
	end
end

% thresh = std_cln .*  -3.5 ;	%%%%%%%% 4
thresh = std_cln .*  -(Threshold) ;	%%%%%%%% 4
reject = std_cln .* -(Threshold*4) ;	%%%%%%%% 20
...
%         plot( Phys(plot_tr,:,ich-1+isp) + (isp*8)*10^-4 ,'Color',[0.4 0.4 0.4])
        plot( Phys(plot_tr,Win(1):Win(2),ich-1+isp) + (isp*8)*10^-4 ,'Color',[0.4 0.4 0.4])
        hold on
        plot([0 size(Phys,2)], [(thresh(ich-1+isp) + (isp*8)*10^-4) (thresh(ich-1+isp) + (isp*8)*10^-4)], '-', 'Color', [0 0.7 0], 'LineWidth',1)
        plot([0 size(Phys,2)], [(reject(ich-1+isp) + (isp*8)*10^-4) (reject(ich-1+isp) + (isp*8)*10^-4)], '-', 'Color', [0.7 0 0], 'LineWidth',1)
