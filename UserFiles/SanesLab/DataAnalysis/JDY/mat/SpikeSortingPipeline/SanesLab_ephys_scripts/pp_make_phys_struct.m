...
%%%%%%%%---JUSTIN---%%%%%%%%
elseif isfield(epData.epocs,'Rate')             % passive recording
    onsets = fs*epData.epocs.Rate.onset;		%samples
elseif isempty(epData.epocs)
	onsets	=	fs*epData.scalars.Pars.ts;
%%%%%%%%%%%%%%%%%%%%%%%%%%%%    
end
...
% Filter data
Wp = [ 300  6000] * 2 / fs;       %cutoff fqs (passband) %300 Hz%
Ws = [ 225  8000] * 2 / fs;       %cutoff fqs (stopband)
%---From UMS---%
% Wp = [ 700  8000] * 2 / fs;			%cutoff fqs (passband)
% Ws = [ 500  10000] * 2 / fs;        %cutoff fqs (stopband)
%--------------%
