function SVMPopulationDecoder
% Try out Support Vector Machine (SVM): Spike Train
% Created 08/14/17
clear all
close all
clc
...
% 		PlotSVMexampleFig(Nogo,Go)
		
		[PercentCorrect,FalseAlarm] = Population_Decoder_SVM_PSTH_v2(Nogo,Go,tau,prop,Dconvolve);
		
% 		[PercentCorrect,FA] = Population_Decoder_SVM_PSTH_v3(Nogo,Go,tau,prop);

% 		[PercentCorrect,FA] = CorrelationCoefficientClassifier(Nogo,Go);
...
if( Save )
	...
	% SavePath	=	['/Users/justinyao/Desktop/DataStore/Population/SVM/FR/'];
% 	SavePath	=	['/Volumes/JUSTIN EXHD/DataStore/PopulationData/Decoder/SVM/SpkTrain/'];
	SavePath    =   ['E:\DataStore\PopulationData\Decoder\SVM\'];
	...
	if( CHL )
		Fname	=	[SavePath 'SVMPSTH-HL.mat'];
	else
		Fname	=	[SavePath 'SVMPSTH-NH.mat'];
	end
...
function [data,Rate,Depth,thresh] = getdata(Path,IDs)
N			=	length(IDs);
data		=	cell(N,1);
Rate		=	cell(N,1);
Depth		=	cell(N,1);
for i=1:N
	...
	[dat,rate,depths,bThresh]	=	gatherdata(files);
	data(i)	=	{dat};
	Rate(i)	=	{rate};
	Depth(i)=	{depths};
	thresh(i)	=	{bThresh};
