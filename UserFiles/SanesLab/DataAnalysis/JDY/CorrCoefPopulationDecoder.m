function CorrCoefPopulationDecoder
% Try out Correlation-based Classifier: Spike Train
% Created 08/18/17
clear all
close all
clc
...
for i=1:NRates
	...
	for j=2:Ndepths
		...
		[PercentCorrect,FalseAlarm] = CorrelationCoefficientClassifier(Nogo,Go,prop);
...
if( Save )
	%---Save Data to File---%
	CorrClass	=	DATA;
    SavePath    =   ['E:\DataStore\PopulationData\Decoder\CorrelationClass\'];
% 	SavePath	=	['/Volumes/JUSTIN EXHD/DataStore/PopulationData/Decoder/CorrelationClass/'];
	...
	if( CHL )
		Fname	=	[SavePath 'CorrClassPSTH-HL.mat'];
	else
		Fname	=	[SavePath 'CorrClassPSTH-NH.mat'];
	end
	
	save( Fname ,'CorrClass')
	disp('Saved Data!')
end
