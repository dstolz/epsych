...
handles.RPfile = 'C:\gits\epsych\UserFiles\SanesLab\RPVdsCircuits\KP\FreqChange.rcx';
...
FMdepths = [0.2 0.5 1]; % 0.05 0.1 0.2 0.5];
% FMdepths = [0];
StartFreq = [1000];
idx=0;  
for ifq = 1:numel(StartFreq)
%     figure(fq); hold on
    
    %set start fq in rpvds    
    handles.RP.SetTagVal('StartFreq',StartFreq(ifq));
    
    %Apply the voltage adjustment for level calibration in RPVds circuit
%     CalAmp = Calibrate(StartFreq(ifq),handles.C);
...
%     handles.RP.SetTagVal('Freq0_norm',handles.C.hdr.cfg.ref.norm); %read norm value from cal file
    ...
    for id = 1:numel(FMdepths)
        ...
        if ((StartFreq(ifq)*FMdepths(id) + StartFreq(ifq)) < 200) || ((StartFreq(ifq)*FMdepths(id) + StartFreq(ifq)) > 20000)
            warning('Skipped stimulus with end frequency out of range')
            sprintf('end fq %i',StartFreq(ifq)*FMdepths(id) + StartFreq(ifq))
            continue
        end
        idx = idx+1;
...
%         handles.RP.SetTagVal('weight_F4', 0);
        ...
        %Set stim params
        AddFreq = StartFreq(ifq) * FMdepths(id);
        handles.RP.SetTagVal('AddFreq',AddFreq);
