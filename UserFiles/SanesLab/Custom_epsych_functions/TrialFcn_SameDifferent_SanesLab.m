function NextTrialID = TrialFcn_SameDifferent_SanesLab(TRIALS)
...
if ~isempty(GUI_HANDLES)
    ...
    num_reminds = str2double(GUI_HANDLES.num_reminds.String{num_reminds_ind});
else
    num_reminds = 5;
end
...
Go_prob = str2double(GUI_HANDLES.go_prob.String{Go_prob_ind});

%Define limit for consecutive nogos
Nogo_lim_ind  =  GUI_HANDLES.Nogo_lim.Value;
Nogo_lim = str2double(GUI_HANDLES.Nogo_lim.String{Nogo_lim_ind});

%Define probability of expected trials
Expected_prob_ind = GUI_HANDLES.expected_prob.Value;
Expected_prob = str2double(GUI_HANDLES.expected_prob.String{Expected_prob_ind});
