function varargout = Appetitive_detection_temporalintegration_GUI(varargin)
...
gui_State = struct('gui_Name',       mfilename, ...
                   ...
                   'gui_OpeningFcn', @Appetitive_detection_temporalintegration_GUI_OpeningFcn, ...
                   'gui_OutputFcn',  @Appetitive_detection_temporalintegration_GUI_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
function Appetitive_detection_temporalintegration_GUI_OpeningFcn(hObject, ~, handles, varargin)
...
%Choose default command line output for Appetitive_detection_temporalintegration_GUI
...
%Disable sound duration dropdown if it's a roved parameter or if it's not a
%parameter tag in the circuit
disabledropdown(handles.am_dur,handles.dev,'AM_Duration')
...
function varargout = Appetitive_detection_temporalintegration_GUI_OutputFcn(hObject, ~, handles) 
...
%If AM Duration is a parameter tag in the circuit
if RUNTIME.UseOpenEx
    if ~isempty(find(ismember(RUNTIME.TDT.devinfo(h.dev).tags,'AM_Duration'),1))
        AM_Duration = AX.GetTargetVal('Behavior.AM_Duration')*fs;
    end
else
    if ~isempty(find(ismember(RUNTIME.TDT.devinfo(h.dev).tags,'AM_Duration'),1))
        AM_Duration = AX.GetTagVal('AM_Duration')*fs;
    end
end
