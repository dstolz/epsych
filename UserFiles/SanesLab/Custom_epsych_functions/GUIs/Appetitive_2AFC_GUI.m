function varargout = Appetitive_2AFC_GUI(varargin)
% GUI for pure tone detection task
%     
% Written by ML Caras Jun 10, 2015
% Modified by JD Yao Jan 27, 2016
...
gui_State = struct('gui_Name',       mfilename, ...
                   ...
                   'gui_OpeningFcn', @Appetitive_2AFC_GUI_OpeningFcn, ...
                   'gui_OutputFcn',  @Appetitive_2AFC_GUI_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
function Appetitive_2AFC_GUI_OpeningFcn(hObject, ~, handles, varargin)
...
%Choose default command line output for Appetitive_2AFC_GUI
...
% realtimeAx = [handles.trialAx,handles.pokeAx,handles.soundAAx,...
%     handles.spoutAx,handles.waterAx,handles.respWinAx];
realtimeAx = [handles.trialAx,handles.pokeAx,handles.soundAAx,...
    handles.soundBAx,handles.spoutAx,handles.spoutBAx,...
    handles.waterAx,handles.respWinAx];
...
% GUI_HANDLES.go_prob = get(handles.SameProb);
% GUI_HANDLES.Nogo_lim = get(handles.DIFFERENTlimit);
...
GUI_HANDLES.remind = 0;
GUI_HANDLES.same_prob = get(handles.SameProb);
GUI_HANDLES.different_lim = get(handles.DIFFERENTlimit);
GUI_HANDLES.trial_filter = get(handles.TrialFilter);
GUI_HANDLES.expected_prob = get(handles.ExpectedProb);
GUI_HANDLES.RepeatDIFFERENT = get(handles.RepeatDIFFERENT);
GUI_HANDLES.num_reminds = get(handles.num_reminds);
...
disabledropdown(handles.freqA,handles.dev,'FreqA')
disabledropdown(handles.freqB,handles.dev,'FreqB')

%Disable FMRateA dropdown if it's a roved parameter or if it's not a
%parameter tag in the circuit
disabledropdown(handles.FMRateA,handles.dev,'FMrateA')
disabledropdown(handles.FMRateB,handles.dev,'FMrateB')


%Disable FMDepthA dropdown if it's a roved parameter or if it's not a
%parameter tag in the circuit
disabledropdown(handles.FMDepthA,handles.dev,'FMdepthA')
disabledropdown(handles.FMDepthB,handles.dev,'FMdepthB')

%Disable AMRateA dropdown if it's a roved parameter or if it's not a
%parameter tag in the circuit
disabledropdown(handles.AMRateA,handles.dev,'AMrateA')
disabledropdown(handles.AMRateB,handles.dev,'AMrateB')

%Disable AMDepthA dropdown if it's a roved parameter or if it's not a
%parameter tag in the circuit
disabledropdown(handles.AMDepthA,handles.dev,'AMdepthA')
disabledropdown(handles.AMDepthB,handles.dev,'AMdepthB')
...
%Disable levelA dropdown if it's a roved parameter or if it's not a
%parameter tag in the circuit
disabledropdown(handles.levelA,handles.dev,'dBSPLA')
disabledropdown(handles.levelB,handles.dev,'dBSPLB')

%Disable sound duration dropdown if it's a roved parameter or if it's not a
%parameter tag in the circuit
disabledropdown(handles.sound_durA,handles.dev,'Stim_DurationA')
disabledropdown(handles.sound_durB,handles.dev,'Stim_DurationB')
...
function varargout = Appetitive_2AFC_GUI_OutputFcn(hObject, ~, handles) 
...
%Capture sound levelA from microphone
capturesound(h);
    ...
    if HITind(end) == 1 && waterupdate < ntrials
        ...
        if RUNTIME.UseOpenEx
            ...
            if AX.GetTargetVal('Behavior.Water_TTL')  == 0 &&...
                    ...
                    AX.GetTargetVal('Behavior.SpoutA_TTL') == 0 &&...
                AX.GetTargetVal('Behavior.SpoutB_TTL') == 0
            ...
            if AX.GetTagVal('Water_TTL')  == 0 &&...
                    ...
                    AX.GetTagVal('SpoutA_TTL') == 0 &&...
                    AX.GetTagVal('SpoutB_TTL') == 0
...
try
    ...
    SAMEind = find(TrialType == 0);
    DIFFERENTind = find(TrialType == 1);
...
% % %         FAind,CRind,SAMEind,DIFFERENTind,variables,...
% % %         ntrials,TrialTypeInd,TrialType,...
% % %         REMINDind,YESind,NOind,expectInd)
    updateResponseHistory(h.DataTable,HITind,MISSind,...
       SAMEind,DIFFERENTind,variables,...
        ntrials,TrialTypeInd,TrialType,...
        REMINDind,YESind,NOind,expectInd)
% % %     %Update FA rate
...
if trial_TTL == 0
    ...
    switch get(handles.sound_durA,'enable')
        case 'on'
            updateSoundDur(handles)
            set(handles.sound_durA,'ForegroundColor',[0 0 1]);
    end
    
    %Update sound frequency and levelA
    updateSoundLevelandFreq(handles)
    ...
    set(handles.SameProb,'ForegroundColor',[0 0 1]);
    set(handles.ExpectedProb,'ForegroundColor',[0 0 1]);
    set(handles.DIFFERENTlimit,'ForegroundColor',[0 0 1]);
...
%UPDATE SOUND LEVELA AND FREQUENCY
...
%If the user has GUI control over the sound levelA, set the levelA in
...
%If the user has GUI control over the FMRateA, set the rate in
%the RPVds circuit to the desired value. Otherwise, simply read the
...
switch get(h.FMRate,'enable')
    ...
    otherwise
        %If FMRateA is a parameter tag in the circuit
...
%If the user has GUI control over the FMDepthA, set the depth in
%the RPVds circuit to the desired value. Otherwise, simply read the
...
switch get(h.FMDepth,'enable')
    ...
    otherwise
        %If FMDepthA is a parameter tag in the circuit
...
%If the user has GUI control over the AMRateA, set the rate in
%the RPVds circuit to the desired value. Otherwise, simply read the
%rate from the circuit directly.
switch get(h.AMRate,'enable')
    ...
    otherwise
        %If AMRateA is a parameter tag in the circuit
...
%If the user has GUI control over the AMDepthA, set the depth in
%the RPVds circuit to the desired value. Otherwise, simply read the
%depth from the circuit directly.
switch get(h.AMDepth,'enable')
    ...
    otherwise
        %If AMDepthA is a parameter tag in the circuit
...
TrialTypeArray(SAMEind) = {'SAME'};
TrialTypeArray(DIFFERENTind) = {'DIFFERENT'};
...
SAMEind = find([D{:,colind}] == 0);
DIFFERENTind = find([D{:,colind}] == 1);
REMINDind = find([D{:,end}] == 1);
YESind = find([D{:,expectind}] == 1);
NOind = find([D{:,expectind}] == 0);

D(SAMEind,colind) = {'SAME'};
D(DIFFERENTind,colind) = {'DIFFERENT'};
...
if ~isempty(DIFFERENTind)
    D{DIFFERENTind,end} = [];
    D{DIFFERENTind,end-1} = [];
end

set(handle,'Data',D)

%UPDATE WATER VOLUME TEXT
%%%Need to updated to add second pumphandle%%%
...
persistent timestamps poke_hist spoutA_hist spoutB_hist soundA_hist soundB_hist water_hist trial_hist response_hist
%light_hist
...
if RUNTIME.UseOpenEx
    spoutA_TTL = AX.GetTargetVal('Behavior.SpoutA_TTL');
    spoutB_TTL = AX.GetTargetVal('Behavior.SpoutB_TTL');
else
    spoutA_TTL = AX.GetTagVal('SpoutA_TTL');
    spoutB_TTL = AX.GetTagVal('SpoutB_TTL');
end
spoutA_hist = [spoutA_hist;spoutA_TTL];
spoutB_hist = [spoutB_hist;spoutB_TTL];
...
if RUNTIME.UseOpenEx
    soundA_TTL = AX.GetTargetVal('Behavior.SoundA_TTL');
    soundB_TTL = AX.GetTargetVal('Behavior.SoundB_TTL');
else
    soundA_TTL = AX.GetTagVal('SoundA_TTL');
    soundB_TTL = AX.GetTagVal('SoundB_TTL');
end
soundA_hist = [soundA_hist;soundA_TTL];
soundB_hist = [soundB_hist;soundB_TTL];
...
spoutA_hist = spoutA_hist(ind);
spoutB_hist = spoutB_hist(ind);
water_hist = water_hist(ind);
soundA_hist = soundA_hist(ind);
soundB_hist = soundB_hist(ind);
...
switch str{val}
    case {'Continuous'}
        ...
        plotContinuous(timestamps,soundA_hist,h.soundAAx,'r',xmin,xmax)
        plotContinuous(timestamps,soundB_hist,h.soundBAx,'r',xmin,xmax)        
        plotContinuous(timestamps,spoutA_hist,h.spoutAAx,'k',xmin,xmax)
        plotContinuous(timestamps,spoutB_hist,h.spoutBAx,'k',xmin,xmax)
    ...
    case {'Triggered'}
        ...
        plotTriggered(timestamps,soundA_hist,poke_hist,h.soundAAx,'r');
        plotTriggered(timestamps,soundB_hist,poke_hist,h.soundBAx,'r');
        plotTriggered(timestamps,spoutA_hist,poke_hist,h.spoutAAx,'k');
        plotTriggered(timestamps,spoutB_hist,poke_hist,h.spoutBAx,'k');
...
function updateIOPlot(h,variables,HITind,SAMEind,REMINDind)
global ROVED_PARAMS RUNTIME
...
if ~isempty(currentdata)
    ...
    %%% SELECT ALL TRIALS NOW...%%%
    keyboard
%     GOtrials = currentdata(SAMEind,:);
    Alltrials = currentdata;
...
%     vals = unique(GOtrials(:,col_ind));
    vals = unique(Alltrials(:,col_ind));
    plotting_data = [];
    
    for i = 1: numel(vals)
%         val_data = GOtrials(GOtrials(:,col_ind) == vals(i),:);
        val_data = Alltrials(Alltrials(:,col_ind) == vals(i),:);
    ...
    switch x_strings{x_ind}
        ...
        case 'dB SPLA'
            xtext = 'Sound Level A (dB SPL)';
        case 'dB SPLB'
            xtext = 'Sound Level B (dB SPL)';            
        case 'FreqA'
            xtext = 'Sound A Frequency (Hz)';
        case 'FreqB'
            xtext = 'Sound B Frequency (Hz)';    
        case 'RespWinDelay'
