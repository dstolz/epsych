function varargout = AMRate_jitter_gui(varargin)
...
% AM rate input vector

%%% based on the AMrate and AMstd set in the protocol, re-write the
%%% AMjitter_trials document with a vector of rates to feed into the
%%% buffer. 
%%%  *issues: indexing could get offset and that would mess up every 
%%%   subsequent trial; must calculate everything based on total duration
%%%   of session / trial dur + isi / num unique stimuli etc...

%%% can create -MAT files for each unique stimulus
